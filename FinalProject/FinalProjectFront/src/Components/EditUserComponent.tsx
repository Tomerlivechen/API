...
import { AdvancedSettingsComponent, AdvancedSettingsComponentProps } from "./AdvancedSettingsComponent";
...
const bioValues : MYFormikValues ={
  ...
  const EditUserComponent: React.FC<{ userInfo: IAppUserDisplay }> = ({ userInfo }) => {
...
const [advanced, setAdvanced] = useState<AdvancedSettingsComponentProps>({userToEdit: initalValues , show: false})
  ...
  const [bannerImageUrl, bannerfile, setbannerImageURL, clearbanner] = useCloudinary();
  ...
  const toggleAdvanced = () => {
    ...
    setAdvanced((prevAdvanced) => ({...prevAdvanced, show: !prevAdvanced.show, userToEdit:userValues}));
  };
  ...
  const handleSubmit = async (values) => {
    ...
    if (HoldImage || HoldBanner)  {
        if(HoldImage){
            await setImageURL(HoldImage).then(() => {setUserValues((prevUserValues =>({...prevUserValues, imageURL:imageUrl })))})
        }
        if(HoldBanner){
            await setbannerImageURL(HoldBanner).then(() => setUserValues((prevUserValues =>({...prevUserValues, banerImageURL:bannerImageUrl}))))
        }
      setIsLoading(true);
    } else {
  ...
  useEffect(() => {
    const submitUser = async () => {
      if ((HoldImage && imageUrl && HoldBanner && bannerImageUrl) || 
...
const handleRemoveBannerImage = ()=>{
    setUserValues((prevUserValues =>({...prevUserValues, banerImageURL:"" })))
}
  ...
  return (
    ...
    <ElementFrame height="800px" width="700px" overflowY="auto" padding="0 pb-4">
    <div className={`text-4xl font-bold  text-center ${colors.ButtonFont}`}>
      ...
      >
        ...
        <div className={`flex items-center  p-2 mt-1 ${userValues.imageURL || HoldImage ? ("pl-3"):("pl-10")}`}>
            <input
              type="file"
              accept="image/*"
...
<div className="flex space-x-4">
          {userValues.imageURL || HoldImage ? (<>
              <FcEditImage  onClick={() => {const fileInput = document.getElementById('image-input');
        if (fileInput) {
          fileInput.click()}}} size={40} className="cursor-pointer " />
          
          <FcRemoveImage size={40} className="cursor-pointer " onClick={handleRemoveImage}/></>):(
                      <FcAddImage onClick={() => {const fileInput = document.getElementById('image-input');
        if (fileInput) {
          fileInput.click()}}} size={40} className="cursor-pointer " />)}</div></div>

                <div className="w-1/2 pl-2 pr-2">
                <p >Banner Picture</p>
                <div className="flex space-x-4">
          {userValues.banerImageURL || HoldBanner ? (<>
              <FcEditImage onClick={() => {const fileInput = document.getElementById('banner-input');
        if (fileInput) {
          fileInput.click()}}} size={40} className="cursor-pointer" />
          
          <FcRemoveImage size={40} className="cursor-pointer" onClick={handleRemoveBannerImage}/></>):(
                      <FcAddImage onClick={() => {const fileInput = document.getElementById('banner-input');
        if (fileInput) {
          fileInput.click()}}} size={40} className="cursor-pointer" />)}
                </div>
                </div>
  ...
  <div className="w-1/3 pr-2 pl-2 flex flex-col items-center">
    ...
    />
      ...
      <div className="flex flex-wrap justify-between mt-5">
      <div className="w-2/3 pr-2 pl-2 flex flex-col items-center">
      </div>
      <div className="w-1/3 pr-2 pl-2 flex flex-col items-center">
